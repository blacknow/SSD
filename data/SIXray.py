...
SIXray_ROOT = "D:\\2019_fall_semester\\01_MachineLearning\\ssd\\data\\VOC_0712\\VOCdevkit\\core_coreless_test"
...
class SIXrayDetection(data.Dataset):
    ...
    def __init__(self, root,
                 image_sets='D:/2019_fall_semester/01_MachineLearning/ssd/data/VOC_0712/VOCdevkit/core_coreless/train.txt',
        ...
        self._annopath = osp.join('%s' % self.root, 'Anno_test', '%s.txt')
        # self._imgpath = osp.join('%s' % self.root, 'Image', '%s.jpg')
        self._imgpath = osp.join('%s' % self.root, 'Image_test', '%s.TIFF')
        ###这尼玛还有小写的tiff？
        self._imgpath1 = osp.join('%s' % self.root, 'Image_test', '%s.tiff')
        self._imgpath_jpg = osp.join('%s' % self.root, 'Image_test', '%s.jpg')
        self.ids = list()
